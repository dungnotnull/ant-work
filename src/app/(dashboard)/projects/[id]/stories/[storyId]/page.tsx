...
export default function StoryDetailPage() {
  ...
  const { data: story, isLoading: storyLoading } = useQuery({
    ...
    queryFn: async () => { const res = await fetch(`/api/stories/${storyId}`); const d = await res.json(); return d.success ? d.data : null; },
  });

  const { data: epic } = useQuery({
    queryKey: ["epic", story?.epic],
    enabled: !!story?.epic,
    queryFn: async () => { const res = await fetch(`/api/epics/${story!.epic}`); const d = await res.json(); return d.success ? d.data : null; },
  });
